...
fn generate_luau(
    ...
    // Stream directly into a String instead of accumulating a Vec<String> and
    // joining — far fewer small allocations on large projects.
    let mut out = String::with_capacity(3 + entries.len() * 6 + 2);

    line(
        &mut out,
        "-- This file was automatically @generated by Tungsten.",
    );
    line(&mut out, "-- It is not intended for manual editing.");
    line(&mut out, "");

    match style {
        "nested" => luau_nested(entries, strip_extension, &mut out, table_name),
        _ => luau_flat(entries, strip_extension, &mut out, table_name),
    }

    line(&mut out, "");
    // No trailing newline after the final `return` — matches the previous
    // `lines.join("\n")` output exactly.
    out.push_str(&format!("return {}", table_name));
    out
}

/// Emit the Luau lines for a DpiGroup value: `key = function(dpiScale) ... end,`.
/// The function body is indented relative to `depth` (the key's indent level).
fn write_dpi_entry(key: &str, variants: &[(u8, u64)], out: &mut String, depth: usize) {
    let ind = "\t".repeat(depth);
    let ind1 = "\t".repeat(depth + 1);
    let ind2 = "\t".repeat(depth + 2);

    line(out, &format!("{}{} = function(dpiScale)", ind, key));
    ...
    for (i, &(scale, id)) in sorted.iter().enumerate() {
        if i == 0 {
            line(out, &format!("{}if dpiScale >= {} then", ind1, scale));
            line(out, &format!("{}return \"rbxassetid://{}\"", ind2, id));
        } else if scale > 1 {
            line(out, &format!("{}elseif dpiScale >= {} then", ind1, scale));
            line(out, &format!("{}return \"rbxassetid://{}\"", ind2, id));
        }
    }

    // The 1x fallback (lowest scale).
    if let Some((_, id)) = sorted.iter().min_by_key(|&&(s, _)| s) {
        line(out, &format!("{}else", ind1));
        line(out, &format!("{}return \"rbxassetid://{}\"", ind2, id));
    }

    line(out, &format!("{}end", ind1));
    line(out, &format!("{}end,", ind));
}

/// Emit the Luau value lines for a single entry at the given indent depth.
/// Shared by the flat and nested writers so both styles emit identical code.
fn write_entry_lines(entry: &CodegenEntry, key: &str, out: &mut String, depth: usize) {
    let ind = "\t".repeat(depth);
    let ind1 = "\t".repeat(depth + 1);
    ...
    match &entry.kind {
        CodegenKind::Sprite {
            ...
            line(out, &format!("{}{} = {{", ind, key));
            line(
                out,
                &format!("{}Image = {},", ind1, entry.asset_ref.luau_string()),
            );
            line(
                out,
                &format!(
        ...
        CodegenKind::Asset => {
            line(
                out,
                &format!("{}{} = {},", ind, key, entry.asset_ref.luau_string()),
            );
        }
        CodegenKind::DpiGroup { variants } => write_dpi_entry(key, variants, out, depth),
    }
}

fn luau_flat(entries: &[CodegenEntry], strip_extension: bool, out: &mut String, table_name: &str) {
    line(out, &format!("local {} = {{", table_name));
    for entry in entries {
        let key = luau_key(entry_key(entry, strip_extension));
        write_entry_lines(entry, &key, out, 1);
    }
    line(out, "}");
}

fn write_luau_tree(tree: &BTreeMap<String, TreeNode>, out: &mut String, depth: usize) {
    let indent = "\t".repeat(depth);
    for (key, node) in tree {
        let qkey = luau_key(key);
        match node {
            TreeNode::Leaf(entry) => write_entry_lines(entry, &qkey, out, depth),
            TreeNode::Branch(subtree) => {
                line(out, &format!("{}{} = {{", indent, qkey));
                write_luau_tree(subtree, out, depth + 1);
...
fn generate_dts(
    ...
    // Stream directly into a String instead of accumulating a Vec<String>.
    let mut out = String::with_capacity(3 + entries.len() * 5 + 4);
    ...
    match style {
        "nested" => dts_nested(entries, strip_extension, &mut out, table_name),
        _ => dts_flat(entries, strip_extension, &mut out, table_name),
    }

    line(&mut out, "");
    // No trailing newline — matches the previous `lines.join("\n")` output.
    out.push_str(&format!("export = {}", table_name));
    out
}

/// Emit the TypeScript declaration lines for a single entry at the given
/// indent depth. Shared by the flat and nested writers.
fn write_dts_entry_lines(entry: &CodegenEntry, key: &str, out: &mut String, depth: usize) {
    let ind = "\t".repeat(depth);
    let ind1 = "\t".repeat(depth + 1);
    ...
    match &entry.kind {
        CodegenKind::Sprite { .. } => {
            line(out, &format!("{}{}: {{", ind, key));
            line(out, &format!("{}Image: string", ind1));
            line(out, &format!("{}ImageRectOffset: Vector2", ind1));
            line(out, &format!("{}ImageRectSize: Vector2", ind1));
            line(out, &format!("{}}}", ind));
        }
        CodegenKind::Asset => line(out, &format!("{}{}: string", ind, key)),
        // DPI functions: (dpiScale: number) => string
        CodegenKind::DpiGroup { .. } => {
            line(
                out,
                &format!("{}{}: (dpiScale: number) => string", ind, key),
            );
        }
    }
}

fn dts_flat(entries: &[CodegenEntry], strip_extension: bool, out: &mut String, table_name: &str) {
    line(out, &format!("declare const {}: {{", table_name));
    for entry in entries {
        let key = ts_key(entry_key(entry, strip_extension));
        write_dts_entry_lines(entry, &key, out, 1);
    }
    line(out, "}");
}

fn write_dts_tree(tree: &BTreeMap<String, TreeNode>, out: &mut String, depth: usize) {
    let indent = "\t".repeat(depth);
    for (key, node) in tree {
        let qkey = ts_key(key);
        match node {
            TreeNode::Leaf(entry) => write_dts_entry_lines(entry, &qkey, out, depth),
            TreeNode::Branch(subtree) => {
                line(out, &format!("{}{}: {{", indent, qkey));
                write_dts_tree(subtree, out, depth + 1);
                line(out, &format!("{}}}", indent));
            }
        }
    }
...
fn dts_nested(entries: &[CodegenEntry], strip_extension: bool, out: &mut String, table_name: &str) {
    let tree = build_tree(entries, strip_extension);
    line(out, &format!("declare const {}: {{", table_name));
    write_dts_tree(&tree, out, 1);
    line(out, "}");
}
...
mod tests {
    ...
    fn sprite_entries() -> Vec<CodegenEntry> {
        vec![
            CodegenEntry::sprite(
                "icons/arrow-up.png".into(),
                AssetRef::Id(12345678),
                (0, 0),
                (48, 48),
            ),
            CodegenEntry::sprite(
                "icons/arrow-down.png".into(),
                AssetRef::Id(12345678),
                (48, 0),
                (48, 48),
            ),
    ...
    fn test_luau_dpi_group_nested() {
        ...
        // DPI entries are function values, not tables: after the `if`'s `end`,
        // the function itself must close with `end,` — never a table brace.
        assert!(
            c.contains("rbxassetid://10\"\n\t\t\tend\n\t\tend,"),
            "DPI function must close with `end,`:\n{c}"
        );
        assert!(
            !c.contains("rbxassetid://10\"\n\t\t\tend\n\t\t},"),
            "DPI function must not close with `}},`:\n{c}"
        );
