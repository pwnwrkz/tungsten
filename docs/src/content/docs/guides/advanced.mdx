...
description: Explore Tungsten's power-user features, including live watch mode, automatic high DPI scaling, performance optimizations, and CI/CD integration.
---
...
To apply metadata to a specific asset, create a file with the same name as the asset, with the `.tmeta` extension.
...
You can set `svg_scale` at the directory level to affect all SVGs within, with per-file overrides available.
...
```lua
local function getHomeIcon(dpiScale)
	if dpiScale >= 3 then
		return "rbxassetid://123"  -- home@3x
	elseif dpiScale >= 2 then
		return "rbxassetid://124"  -- home@2x
	else
		return "rbxassetid://125"  -- home
	end
end
```
...
For teams or larger projects, you should avoid syncing assets from individual local machines. Tungsten is designed for CI/CD integration.
...
Control parallel uploads via `tungsten.toml`:

```toml title="tungsten.toml"
max_concurrent_uploads = 20  # Default: 10
```

Increase for high-bandwidth connections; decrease if hitting Roblox rate limits.

### Incremental Sync

Tungsten maintains a `tungsten.lock.toml` lockfile that tracks uploaded assets by content hash. On subsequent runs, unchanged assets are skipped entirely — no re-upload, no re-processing.

To force a full re-sync, delete the lockfile:

```bash title="Terminal"
rm tungsten.lock.toml
```
